...
namespace nerp
{
    int Vocabulary::ParseEngWords(char* sentence, char** words, int max)
    {
        char *word;
        int i = 0;
    ...
    int Vocabulary::GetAndInsertWordsIndics(char** words, int* wids, int max)
    {
        int i;
    ...
    int Vocabulary::GetAndInsertWordsIndics(char** words, vector<int> &wids, int max)
    {
        int i;
        for (i = 0; i < max && words[i]!=0 && words[i][0] != 0; i++) 
        {
    ...
    int Vocabulary::GetWordIndics(char** words, int *wids, int max, int &UnkownNum)
    {
        int i;
        UnkownNum = 0;
    ...
    int * Vocabulary::reverse(int *words)
    {
    ...
    int Vocabulary::length(const int *words)
    {
        unsigned int len = 0;
    ...
    bool Vocabulary::WriteVocabIndex(FILE * file)
    {
    ...
    bool Vocabulary::WriteVocabText(FILE * file)
    {
        map<string, int>::iterator iter;
        for (iter = index_data.begin(); iter != index_data.end(); ++iter)
        {
